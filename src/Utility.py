...
class Utility:
    ...
    def getNeighbour(state: State, coordinate1: List[int], coordinate2: List[int]) -> State:
        neighbour_cube = state.cube.copy()
        
        value1 = neighbour_cube[coordinate1[0], coordinate1[1], coordinate1[2]]
        value2 = neighbour_cube[coordinate2[0], coordinate2[1], coordinate2[2]]
    ...
    def getRandomSuccessor(state: State) -> State:
        coordinate_pairs = Utility.generateCoordinatePairs(state.dim)
